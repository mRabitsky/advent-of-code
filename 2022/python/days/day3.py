...
def _ascii_sum(arr):
    return sum(ord((char := (ft.reduce(op.and_, r)).pop())) - (38 if char.isupper() else 96) for r in arr)


def part1():
    return _ascii_sum((set(r[:len(r) // 2]), set(r[len(r) // 2:])) for r in rucksacks)


def part2():
    return _ascii_sum(it.ichunked(map(set, rucksacks), 3))
