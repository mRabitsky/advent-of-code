...
67: 81 92 | 10 13
32: 50 13 | 104 92"""
messages = """bbabbaaaabbaabbbaabbabaabbaabbaababaabab
...
rules = sorted(rules.strip().split('\n'))
...
def expand(i):
    if isinstance(g[i], str):
        return g[i]
    g[i] = '(' + '|'.join(['(' + ''.join([expand(j) for j in option]) + ')' for option in g[i]]) + ')'
    return g[i]
...
def expand2(i):
    ...
    if i == '8':
        ...
        return g[i]
    if i == '11':
        g[i] = '(' + expand2('42') + "(?n)?" + expand2('31') + ')'
        return g[i]
    g[i] = '(' + '|'.join(['(' + ''.join([expand2(j) for j in option]) + ')' for option in g[i]]) + ')'
    return g[i]


def part1():
    expand('0')
    return sum([1 if re.match('^' + g['0'] + '$', m) else 0 for m in messages])


def part2():
    expand2('0')
    counter = g['0'][:g['0'].find(g['11'])].count('(')
    g['0'] = re.sub('n', str(counter + 1), g['0'])
    return sum([1 if pcre.match('^' + g['0'] + '$', m) else 0 for m in messages])
