...
string = """dark maroon bags contain 2 striped silver bags, 4 mirrored maroon bags, 5 shiny gold bags, 1 dotted gold bag.
...
ls = [line[0:-1] for line in ls]

g = dict()
for line in ls:
    x = line.split(' contain ')
    g[' '.join(x[0].split(' ')[:2])] = [(' '.join(bag.strip().split(' ')[1:3]), {'w': int(bag.strip().split(' ')[:1][0])}) for bag in x[1].split(', ') if bag != 'no other bags']

dg = nx.DiGraph()
for k in g.keys():
    dg.add_node(k)
for k in g.keys():
    dg.add_edges_from([(k, *v) for v in g[k]])


def part1():
    return len(list(filter(lambda k: k != 'shiny gold' and nx.has_path(dg, k, 'shiny gold'), g.keys())))


def path_length(v, u):
    w = dg.get_edge_data(u, v, {'w': 0})['w']
    if len(dg[v]) == 0:
        return w
    return w + w * sum([path_length(t, v) for t in dg[v]])


def part2():
    return sum([path_length(v, 'shiny gold') for v in dg['shiny gold']])
