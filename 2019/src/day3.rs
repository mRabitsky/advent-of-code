...
fn part_one(a: String, b: String) -> isize {
    ...
    a.split(",").for_each(|s| {
        ...
        match s.chars().nth(0) {
            ...
            Some('U') => {
                line_a.push((curr_point.0, curr_point.1, curr_point.0, curr_point.1 + distance));
            ...
            _   => println!("we have a problem here")
        };
    });
    curr_point = (0, 0);
    b.split(",").for_each(|s| {
        ...
        match s.chars().nth(0) {
            Some('R') => {
                line_b.push((curr_point.0, curr_point.1, curr_point.0 + distance, curr_point.1));
                curr_point.0 += distance;
            },
            Some('L') => {
                line_b.push((curr_point.0, curr_point.1, curr_point.0 - distance, curr_point.1));
            ...
            Some('U') => {
                line_b.push((curr_point.0, curr_point.1, curr_point.0, curr_point.1 + distance));
                curr_point.1 += distance;
            },
            Some('D') => {
                line_b.push((curr_point.0, curr_point.1, curr_point.0, curr_point.1 - distance));
    ...
    line_a.iter().for_each(|a| {
        line_b.iter().for_each(|b| {
            let c = (a.2 - a.0, a.3 - a.1);
            let d = (b.2 - b.0, b.3 - b.1);
            let s = (-c.1 as f64 * (a.0 as f64 - b.0 as f64) + c.0 as f64 * (a.1 as f64 - b.1 as f64)) / (-d.0 as f64 * c.1 as f64 + c.0 as f64 * d.1 as f64);
            let t = ( d.0 as f64 * (a.1 as f64 - b.1 as f64) - d.1 as f64 * (a.0 as f64 - b.0 as f64)) / (-d.0 as f64 * c.1 as f64 + c.0 as f64 * d.1 as f64);
            if s >= 0.0 && s <= 1.0 && t >= 0.0 && t <= 1.0 {
                intersections.push(((a.0 as f64 + t * c.0 as f64) as isize, (a.1 as f64 + t * c.1 as f64) as isize));
            }
        });
    });
    
    intersections.retain(|&x| x != (0, 0));
    let mut results: Vec<isize> = intersections.iter().map(|x| x.0.abs() + x.1.abs()).collect();
    results.sort_unstable_by(|f1, f2| f1.partial_cmp(f2).unwrap());
    
    results[0]
}
fn part_two(a: String, b: String) -> usize {
    use std::collections::HashMap;
    
    let mut line_a: HashMap<(isize, isize, isize, isize), usize> = HashMap::new();
    let mut line_b: HashMap<(isize, isize, isize, isize), usize> = HashMap::new();
    let mut intersections: HashMap<(isize, isize), usize> = HashMap::new();
    let mut curr_point: (isize, isize) = (0, 0);
    ...
    a.split(",").for_each(|s| {
        ...
        match s.chars().nth(0) {
            Some('R') => {
                line_a.insert((curr_point.0, curr_point.1, curr_point.0 + distance, curr_point.1), steps);
                curr_point.0 += distance;
            },
            Some('L') => {
                line_a.insert((curr_point.0, curr_point.1, curr_point.0 - distance, curr_point.1), steps);
            ...
            Some('U') => {
                line_a.insert((curr_point.0, curr_point.1, curr_point.0, curr_point.1 + distance), steps);
                curr_point.1 += distance;
            },
            Some('D') => {
                line_a.insert((curr_point.0, curr_point.1, curr_point.0, curr_point.1 - distance), steps);
    ...
    b.split(",").for_each(|s| {
        ...
        match s.chars().nth(0) {
            Some('R') => {
                line_b.insert((curr_point.0, curr_point.1, curr_point.0 + distance, curr_point.1), steps);
                curr_point.0 += distance;
            },
            Some('L') => {
                line_b.insert((curr_point.0, curr_point.1, curr_point.0 - distance, curr_point.1), steps);
            ...
            Some('U') => {
                line_b.insert((curr_point.0, curr_point.1, curr_point.0, curr_point.1 + distance), steps);
                curr_point.1 += distance;
            },
            Some('D') => {
                line_b.insert((curr_point.0, curr_point.1, curr_point.0, curr_point.1 - distance), steps);
    ...
    line_a.keys().for_each(|a| {
        line_b.keys().for_each(|b| {
            let c = (a.2 - a.0, a.3 - a.1);
            let d = (b.2 - b.0, b.3 - b.1);
            let s = (-c.1 as f64 * (a.0 as f64 - b.0 as f64) + c.0 as f64 * (a.1 as f64 - b.1 as f64)) / (-d.0 as f64 * c.1 as f64 + c.0 as f64 * d.1 as f64);
            let t = ( d.0 as f64 * (a.1 as f64 - b.1 as f64) - d.1 as f64 * (a.0 as f64 - b.0 as f64)) / (-d.0 as f64 * c.1 as f64 + c.0 as f64 * d.1 as f64);
            if s >= 0.0 && s <= 1.0 && t >= 0.0 && t <= 1.0 {
                let key = ((a.0 as f64 + t * c.0 as f64) as isize, (a.1 as f64 + t * c.1 as f64) as isize);
                if !intersections.contains_key(&key) {
                    intersections.insert(key, (*line_a.get(a).unwrap() as isize - ((a.2 - key.0).abs() + (a.3 - key.1).abs()) + *line_b.get(b).unwrap() as isize - ((b.2 - key.0).abs() + (b.3 - key.1).abs())) as usize);
                }
            }
        });
    });
    ...
    let mut results: Vec<&usize> = intersections.values().collect();
