...
fn PART_ONE(a: String, b: String) -> isize {
    let mut lineA: Vec<(isize, isize, isize, isize)> = vec![];
    let mut lineB: Vec<(isize, isize, isize, isize)> = vec![];
    let mut intersections: Vec<(isize, isize)> = vec![];
    let mut currPoint: (isize, isize) = (0, 0);
    
    a.split(",").for_each(|s| {
        let distance: isize = s.get(1..).unwrap().parse().unwrap();
        match s.chars().nth(0) {
            Some('R') => {
                lineA.push((currPoint.0, currPoint.1, currPoint.0 + distance, currPoint.1));
                currPoint.0 += distance;
            },
            Some('L') => {
                lineA.push((currPoint.0, currPoint.1, currPoint.0 - distance, currPoint.1));
                currPoint.0 -= distance;
            ...
            Some('U') => {
                lineA.push((currPoint.0, currPoint.1, currPoint.0, currPoint.1 + distance));
                currPoint.1 += distance;
            },
            Some('D') => {
                lineA.push((currPoint.0, currPoint.1, currPoint.0, currPoint.1 - distance));
                currPoint.1 -= distance;
    ...
    b.split(",").for_each(|s| {
        ...
        match s.chars().nth(0) {
            Some('R') => {
                lineB.push((currPoint.0, currPoint.1, currPoint.0 + distance, currPoint.1));
                currPoint.0 += distance;
            },
            Some('L') => {
                lineB.push((currPoint.0, currPoint.1, currPoint.0 - distance, currPoint.1));
            ...
            Some('U') => {
                lineB.push((currPoint.0, currPoint.1, currPoint.0, currPoint.1 + distance));
                currPoint.1 += distance;
            },
            Some('D') => {
                lineB.push((currPoint.0, currPoint.1, currPoint.0, currPoint.1 - distance));
    ...
    lineA.iter().for_each(|a| {
        lineB.iter().for_each(|b| {
...
fn PART_TWO(a: String, b: String) -> usize {
    ...
    let mut lineA: HashMap<(isize, isize, isize, isize), usize> = HashMap::new();
    let mut lineB: HashMap<(isize, isize, isize, isize), usize> = HashMap::new();
    ...
    a.split(",").for_each(|s| {
        ...
        match s.chars().nth(0) {
            Some('R') => {
                lineA.insert((currPoint.0, currPoint.1, currPoint.0 + distance, currPoint.1), steps);
                currPoint.0 += distance;
            },
            Some('L') => {
                lineA.insert((currPoint.0, currPoint.1, currPoint.0 - distance, currPoint.1), steps);
            ...
            Some('U') => {
                lineA.insert((currPoint.0, currPoint.1, currPoint.0, currPoint.1 + distance), steps);
                currPoint.1 += distance;
            },
            Some('D') => {
                lineA.insert((currPoint.0, currPoint.1, currPoint.0, currPoint.1 - distance), steps);
    ...
    b.split(",").for_each(|s| {
        ...
        match s.chars().nth(0) {
            Some('R') => {
                lineB.insert((currPoint.0, currPoint.1, currPoint.0 + distance, currPoint.1), steps);
                currPoint.0 += distance;
            },
            Some('L') => {
                lineB.insert((currPoint.0, currPoint.1, currPoint.0 - distance, currPoint.1), steps);
            ...
            Some('U') => {
                lineB.insert((currPoint.0, currPoint.1, currPoint.0, currPoint.1 + distance), steps);
                currPoint.1 += distance;
            },
            Some('D') => {
                lineB.insert((currPoint.0, currPoint.1, currPoint.0, currPoint.1 - distance), steps);
    ...
    lineA.keys().for_each(|a| {
        lineB.keys().for_each(|b| {
            let c = (a.2 - a.0, a.3 - a.1);
            let d = (b.2 - b.0, b.3 - b.1);
            let s = (-c.1 as f64 * (a.0 as f64 - b.0 as f64) + c.0 as f64 * (a.1 as f64 - b.1 as f64)) / (-d.0 as f64 * c.1 as f64 + c.0 as f64 * d.1 as f64);
            let t = ( d.0 as f64 * (a.1 as f64 - b.1 as f64) - d.1 as f64 * (a.0 as f64 - b.0 as f64)) / (-d.0 as f64 * c.1 as f64 + c.0 as f64 * d.1 as f64);
            if s >= 0.0 && s <= 1.0 && t >= 0.0 && t <= 1.0 {
                ...
                if !intersections.contains_key(&key) {
                    intersections.insert(key, (*lineA.get(a).unwrap() as isize - ((a.2 - key.0).abs() + (a.3 - key.1).abs()) + *lineB.get(b).unwrap() as isize - ((b.2 - key.0).abs() + (b.3 - key.1).abs())) as usize);
                }
            }
        });
    });
